...
import com.ray.beer.data.repository.local.SharedPreferencesManager
        ...
        if (BuildConfig.DEBUG) {
            ...
            if (FlipperUtils.shouldEnableFlipper(this)) {
                AndroidFlipperClient.getInstance(this).run {
                    addPlugin(InspectorFlipperPlugin(this@BeerApplication, DescriptorMapping.withDefaults()))
                    addPlugin(NetworkFlipperPlugin())
                    addPlugin(NavigationFlipperPlugin.getInstance())
                    addPlugin(DatabasesFlipperPlugin(this@BeerApplication))
                    addPlugin(CrashReporterPlugin.getInstance())
                    addPlugin(LeakCanary2FlipperPlugin())
                    addPlugin(
                        SharedPreferencesFlipperPlugin(
                            this@BeerApplication,
